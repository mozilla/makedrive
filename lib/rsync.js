...
// See if a path is marked unsync'ed upstream.
...
// Remove the unsynced metadata from a file
function removeUnsyned(fs, path, callback) {
...
rsync.patch = function(fs, path, diff, options, callback) {
  ...
  function removeNodes(path, entryDiff, callback) {
    ...
    fs.readdir(path, function(err, destContents) {
      ...
      function maybeUnlink(item, callback) {
        ...
        isPathUnsynced(fs, deletePath, function(err, unsynced) {
          if(err) {
            return callback(err);
          }
  ...
  function syncEach(entry, callback) {
    ...
    fs.readFile(syncPath, function(err, data) {
      ...
      isPathUnsynced(fs, syncPath, function(err, unsynced) {
        if(err) {
          return callback(err);
        }
        ...
        if(unsynced) {
          conflict.makeConflictedCopy(fs, syncPath, function(err) {
            ...
            removeUnsyned(fs, syncPath, function(err) {
              if(err) {
                return handleError(err, callback);
              }
