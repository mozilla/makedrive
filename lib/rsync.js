...
rsync.sourceList = function(fs, path, options, callback) {
  ...
  if(!validateParams(fs, path, callback)) return;
  ...
  function SourceNode(path, stats) {
    ...
    sourceNode.path = path;
    sourceNode.modified = stats.mtime;
    sourceNode.size = stats.size;
    sourceNode.type = stats.type; 
  }
...
rsync.checksums = function(fs, path, srcList, options, callback) {
  ...
  function getChecksumsForSourceNode(sourceNode, callback) {
    ...
    function createParentDirectories() {
      // fs.Shell().mkdirp(Path.dirname(sourceNode.path), function(err) {
...
rsync.diff = function(fs, path, checksumList, options, callback) {
  ...
  function DiffNode(path, type, modifiedTime, diffs) {
    ...
    diffNode.path = path;
    diffNode.type = type;
    diffNode.diffs = diffs;
    diffNode.modified = modifiedTime;
  }
